function [suitability inlierIndices] = pcdenoise_normal(ptCloudIn, varargin)
...
normals = ptCloudIn.Normal';

% Compute the mean distance to neighbors for every point, and exclude the
% query point itself
[index, dists, valids] = multiQueryKNNSearchImpl(ptCloudIn, points, numNeighbors+1);
...
for j = 1:length(normals)
    self=normals(:,j);
    nnormals= normals(:,index(:,j));
    
    for k = 1:numNeighbors+1
        ndists(k,j) =norm(self-nnormals(:,k));
    end
...
actualNums = double(max(valids(:))-1);
meanDist = sum(ndists(2:actualNums, :),1)/actualNums;
...
% Select the inliers
tfn = meanDist <= distThreshold;




suitability =tf&tfn;
inlierIndices = uint32(find(suitability));
